...
```
   ...
   ┌────▼─────┐   ┌────▼─────┐   ┌────▼─────┐
   │ Next.js  │   │ /blog/*  │   │ /admin/* │
   │ 15 App   │   │ proxy to │   │ Payload  │
   │          │   │ WordPress│   │ CMS      │
   │ Landing  │   │ (during  │   │ Admin    │
   │ Pages    │   │ transit) │   │ Panel    │
   └────┬─────┘   └──────────┘   └────┬─────┘
        │                              │
   ┌────┼─────┐                   ┌────▼─────┐
...
shadcn Tailw Blocks                    │
 /ui   CSS   (Hero,            ┌───────┼───────┐
             CTA,              │       │       │
             Feature,     ┌────▼──┐┌───▼──┐┌───▼───┐
             Pricing,     │Claude ││Admin ││WebMCP │
             Blog,        │Code   ││Users ││(future│
             Form,        │       ││      ││       │
             etc.)        │Vibe   ││Edit  ││AI     │
                          │code + ││in    ││agents │
                          │upload ││browser│       │
   ...
   │  │• Tags      │    │• Any files       │  │
   │  │• Users     │    │                  │  │
   │  │• Forms     │    │ Auto-thumbnails  │  │
   │  │• SEO Data  │    │ via Payload      │  │
   │  │• Navigation│    │                  │  │
   │  │• Settings  │    │                  │  │
   │  └────────────┘    └──────────────────┘  │
   └──────────────────────────────────────────┘
   ...
   │       3-Layer Guardrails                 │
   │                                          │
   │  shadcn/ui    Payload CMS   CLAUDE.md    │
   │  Constrains   Constrains    Constrains   │
   │  UI design    content       code         │
   │               structure     patterns     │
   └──────────────────────────────────────────┘
   ...
   │                                          │
   │  Pages          Posts         Media      │
   │  ├ title        ├ title       ├ file→GCP │
   │  ├ slug         ├ slug        ├ alt text │
   │  ├ parent       ├ content     ├ sizes    │
   │  ├ template     ├ categories  └ thumbs   │
   │  ├ cover image  ├ tags                   │
   │  ├ meta (SEO)   ├ author     Navigation  │
   │  └ blocks[]     ├ cover img  ├ menu items│
   │    ├ Hero       └ meta(SEO)  └ links     │
   │    ├ CTA                                 │
   ...
   │  ├ Create blocks ────→ Git repo          │
   │  ├ Upload files ─────→ Payload API → GCP │
   │  ├ Create pages ─────→ Payload API → DB  │
   │  ├ SEO audit & fix ──→ Payload API → DB  │
   │  └ Structured data ──→ Next.js code      │
   │                                          │
   │  Admin Panel (browser, no code)          │
   │  ├ Edit page content (text, images)      │
   │  ├ Write blog posts (rich text)          │
   │  ├ Upload media                          │
   │  ├ Manage navigation                     │
   │  └ Publish / draft / schedule            │
   │                                          │
   │  WebMCP (future)                         │
   │  ├ AI agents create/update via MCP       │
